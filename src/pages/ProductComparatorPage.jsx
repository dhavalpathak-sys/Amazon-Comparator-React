  ...
  try {
    ...
    if (storedGoldenProduct) {
      ...
      const description = parsedProduct.description ||
                        (parsedProduct.features && parsedProduct.features.length > 0 ? parsedProduct.features.join(' Ã¢â‚¬Â¢ ') : '') ||
                        '';
...
const handleLoadUrl = async () => {
  ...
  try {
    ...
    console.log('Ã°Å¸â€Â Scraped Product Data:', data);
    setScrapedData(data);
    ...
    setOriginalTitle(data.title || '');
     const description = data.description ||
                       (data.features && data.features.length > 0 ? data.features.join(' Ã¢â‚¬Â¢ ') : '') ||
                       '';
    setProductDescription(description);
    setAcceptedDescription(description);
    setOriginalDescription(description);
...
const getGoldenDescription = () => {
  ...
  return goldenProduct.description ||
         (goldenProduct.features && goldenProduct.features.length > 0 ? goldenProduct.features.join(' Ã¢â‚¬Â¢ ') : '') ||
...
const handleProceed = () => {
  ...
  if (allAccepted && scrapedData) {
    ...
    console.log('Ã°Å¸â€™Â¾ Stored existing product in localStorage:', existingProduct);
                ...
                {goldenProduct.details ? (
                  ...
                  (() => {
                    ...
                    const normalizeKey = (key) => {
                      ...
                      return key.trim()
                        .replace(/[\n\r\s:Ã¢â‚¬ÂÃ¢â‚¬Å½]+/g, '') // Remove newlines, spaces, colons, and special Unicode chars
                        .toLowerCase();
                    };
